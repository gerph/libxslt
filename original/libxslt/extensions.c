...
xsltInitCtxtExts(xsltTransformContextPtr ctxt)
{
    ...
    while (style != NULL) {
        if (style->extInfos != NULL) {
            xmlHashScan(style->extInfos, (xmlHashScanner)xsltInitCtxtExt, &ctx);
            if (ctx.ret == -1)
                return (-1);
        }
...
xsltUnregisterExtModule(const xmlChar * URI)
{
    ...
    ret = xmlHashRemoveEntry(xsltExtensionsHash, URI, (xmlHashDeallocator)xsltFreeExtModuleEntry);
...
xsltUnregisterAllExtModules(void)
{
    ...
    xmlHashFree(xsltExtensionsHash, (xmlHashDeallocator)xsltFreeExtModuleEntry);
    xsltExtensionsHash = NULL;
                             ...
                             xsltTransformFunction transform)
{
    ...
    xmlHashUpdateEntry2(xsltElementsHash, name, URI, (void *) ext,
                        (xmlHashDeallocator)xsltFreeExtElementEntry);
...
xsltUnregisterAllExtModuleElement(void)
{
    ...
    xmlHashFree(xsltElementsHash, (xmlHashDeallocator)xsltFreeExtElementEntry);
...
xsltCleanupGlobals(void)
{
    ...
    if (NULL != xsltModuleHash) {
        xmlHashScan(xsltModuleHash, (xmlHashScanner)xsltHashScannerModuleFree, 0);
